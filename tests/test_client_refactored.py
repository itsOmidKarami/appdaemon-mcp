...
from appdaemon_mcp.client import AppDaemonClient, ADAuthError, ADNotFoundError
from appdaemon_mcp.models import AppInfo, AppEntity, LogEntry
...
async def test_get_info_returns_appinfo(client: AppDaemonClient):
    with aioresponses() as m:
        m.get(f"{BASE_URL}/api/appdaemon", payload={"data": {"version": "4.4.2", "timezone": "UTC"}})
...
async def test_get_state_returns_appentity(client: AppDaemonClient):
    with aioresponses() as m:
        m.get(f"{BASE_URL}/api/appdaemon/state/default/light.kitchen", payload={"data": {"state": "on", "entity_id": "light.kitchen"}})
...
async def test_get_logs_returns_logentries(client: AppDaemonClient):
    with aioresponses() as m:
        m.get(f"{BASE_URL}/api/appdaemon/logs", payload={"data": [{"ts": 1000.0, "type": "INFO", "message": "Test"}]})
